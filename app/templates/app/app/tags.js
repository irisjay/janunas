...
riot.tag2('content-service-instructor-course-add', '<model-themer> <fieldset> <label>Course name:<input name="course_name" type="text" placeholder="Wow. Such course."></label> <label>Course description:<textarea name="component_description" cols="40" rows="3" placeholder="Much course"></textarea></label> </fieldset> <button type="submit" name="add_course"> Add course! </button> </model-themer>', '', '', function(opts) {
	(function (self) {
	    self .on ('mount', function () {
	        var course_name = self .root .querySelector ('input[name="course_name"]');
	        var component_description = self .root .querySelector ('input[name="component_description"]');
...
riot.tag2('content-service-participant-my-course', '<model-themer> <h2>{((recalls (\'course\') || {}) .course || {}) .name}</h2> <p>{((recalls (\'course\') || {}) .course || {}) .description}</p> <table> <tr><th>Modules</th></tr> <tr each="{assemble (((recalls (\'course\') || {}) .course || {}) .module_set) && assemble (((recalls (\'course\') || {}) .course || {}) .module_set) .slice (0, assemble (((recalls (\'course\') || {}) .course || {}) .module_set) .filter (new Function (⁗module⁗,⁗ return module .done ⁗)) .length + 1)}"><td><a href="#service/participant/my/module/#{recalls (\'course_id\')}/{order}" if="{done}"><strike>{name}</strike></a ><a href="#service/participant/my/module/#{recalls (\'course_id\')}/{order}" if="{! done}">{name}</a ></td ></tr> <tr if="{assemble (((recalls (\'course\') || {}) .course || {}) .module_set) && assemble (((recalls (\'course\') || {}) .course || {}) .module_set) .filter (new Function (⁗module⁗,⁗ return ! module .done ⁗)) .length > 1}"><td><i>find out later......</i></td></tr> </table> </model-themer>', '', '', function(opts) {
	(function (self) {
	    self .stuff .on ('course', function () {
	        self .update ();
	    });
...
riot.tag2('model-themer', '<section> <header>SPD</header> <article><yield></yield></article> <footer><button name="logout">Logout</button><button name="service">Services</button></footer> </section>', '', '', function(opts) {
	(function (self) {
	    util .logout (self);
