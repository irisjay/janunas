...
def categories(request):
    try:
        ...
        data = [cat.name for cat in Category.objects.all()]
        
        return JsonResponse(data, safe=False)
    except Exception as error:
        return JsonResponse({'error': repr(error)}, safe=False)
...
def my_course_new(request):
    try:
        ...
        course_category = request.META['HTTP_X_COURSE_CATEGORY']
